...
export class GitBook {
  ...
  get summaryFilePath () {
    return path.join(this.bookRootPath, this.summaryFileName)
  }

  get readmeFilePath () {
    return path.join(this.bookRootPath, this.readmeFileName)
  }
  ...
  getBookAst () {
    ...
    // Prepend readme chapter manually if not specified by book author
    // This is an undocumented feature from original GitBook implementation
    if (!this.readmeIncluded) {
      const readmeChapter =
        new ChapterNode('Introduction', this.readmeFilePath)
      this.bookAst.children.unshift(readmeChapter)
    }
    return this.bookAst
  }
  ...
  toChapterAst (liNode) {
    ...
    if (absFilePath === this.readmeFilePath) {
      this.readmeIncluded = true
    }
